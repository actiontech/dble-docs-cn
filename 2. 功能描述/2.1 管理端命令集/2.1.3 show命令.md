...
### 2.1.3.10  show @@datasource.syndetail where name=？
 show @@datasource.syndetail where name=xxx；其中，xxx为要查看的datasource的名字。
...
描述：展示结果集大小超过某个阈值(默认512K，可以通过maxResultSet配置，最好为bufferPoolPageSize整数倍) 的sql，结果集统计信息
...
show @@heartbeat.detail where name=xxx; 其中，xxx为要查询的datasource的名字。
...
show @@directmemory=N；其中，N有两个取值1或者2。
