...
+------+----------------+---------------+--------+------+------+---------+---------------+
| NAME | DBGROUP        | SCHEMA_EXISTS | ACTIVE | IDLE | SIZE | EXECUTE | RECOVERY_TIME |
+------+----------------+---------------+--------+------+------+---------+---------------+
...
READ_LOAD: 读负载 (请求次数)
WRITE_LOAD: 写负载 (请求次数)
DISABLED: schema.xml中dbinstance中的配置 (2.19.09.0以前的版本没有此列，disabled为true的结点不显示)
```
...
前提条件: heartbeat 配置了 show slave status（参见1.2 schemal.xml）  
