...
21. 复杂查询会透传难以理解的报错，建议结合explain语句分析 
原因 ：需要枚举所有错误来做替换.成本高，收益低
相关issue ：https://github.com/actiontech/dble/issues/1449

22. 开启 enableCursor 选项后，所有的 prepareStatement 的 prepare 阶段均需要向 mysql 执行特殊语句，用来获取结果集的列数并返回给前端，这个执行会损失一部分性能（即使客户端没有开启游标）。

23. 开启XA功能后，不支持执行含有隐式提交的sql
