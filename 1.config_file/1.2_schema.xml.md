...
注意：如果是使用通配符的配置，那么dataNode(name)的通配符展开个数必须等于dataHost通配符展开个数与database通配符展开个数之积,上例中,name的个数为20,dataHost的个数为10,database的个数为2;又例如<dataNode name="dn,dn1$0-19,dnx" dataHost="localhost,localhost1$0-9" database="db1$0-1" />中,name的个数为22,dataHost的个数为11,database的个数为2。
...
<tr>
<td class="tg-yw4l">incrementColumn</td>
