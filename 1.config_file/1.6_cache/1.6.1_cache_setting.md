...
\#default cache and special table, the table which is not special will use default cache  
layedpool.TableID2DataNodeCache=**type**,**max_size**,**expire_seconds**  
\# special table :table conf format is 
```
`schema`_`table`
``` 
...
\#pattern 2:   
\#only special table, other table will not use cache  
