...
<td >否</td>
<td >默认0，否</td>
<td >心跳超时阈值 。1.心跳时候检会查上次是不正常的心跳,如果无返回，并且距离最近的正常心跳的时间大于timeout，则标记该结点不可达。2. 连续返回失败时，每次都会检查是否超时，如果第一次失败到当前时间大于timeout秒 ，标记该结点不可达。 </td>
</tr>
<tr>
...
<td >否</td>
<td >默认0，否</td>
<td >心跳失败后。开始重试errorRetryCount次，重试期间成功，则标记回OK。(目的：防止网络抖动或者连接异常断开场景), 重试期间超时，按照超时逻辑处理。 </td>
</tr>  
</table>
...
<td >否</td>
<td >-1</td>
<td >在SQL执行之前会根据这个添加限制，如果SQL中已有限制则无效</td>
</tr>
<tr>
<td >table</td>
<td >详见table配置选项</td>
