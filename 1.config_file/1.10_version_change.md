## 1.10 版本变更
+ [3.23.08.0](#config-title-3.23.08.0)
  - [配置文件调整](#config-update-3.23.08.0)
  - [管理端命令调整](#manager-cmd-3.23.08.0)
  - [sql_log表结构调整](#update-sql_log-3.23.08.0)
+ [3.23.04.0](#config-title-3.23.04.0)
  - [配置文件调整](#manager-cmd-3.23.04.0)
+ [3.22.11.0](#config-title-3.22.11.0)
  - [配置文件调整](#manager-cmd-3.22.11.0)
...
###  <span id="config-title-3.23.08.0">3.23.08.0 变更内容</span>
####  <span id="config-update-3.23.08.0">1 配置调整</span>
##### 1.1 bootstrap.cnf
##### 1.1.1 sql统计相关的变动
<table>
<tbody>
...
<tr>
<td  ><p><strong>移除</strong></p></td>
<td  ><p><strong>useSqlStat</strong></p></td>
<td  ><p><strong>3.23.08</strong></p></td>
<td  ><p><strong>不兼容</strong></p></td>
<td  ><p><strong>原useSqlStat=0，现在由samplingRate=0代替；原useSqlStat=1，现在由samplingRate=100代替；</strong></p></td>
</tr>
<tr>
<td  ><p><strong>移除</strong></p></td>
<td  ><p><strong>bufferUsagePercent</strong></p></td>
<td  ><p><strong>3.23.08</strong></p></td>
<td  ><p><strong>不兼容</strong></p></td>
<td  ><p><strong>原来默认达到内存的80%，则将产生sql统计数据进行清理，达到释放部分内存的目的（原配合useSqlStat=1使用）</strong></p></td>
</tr>
<tr>
<td  ><p><strong>移除</strong></p></td>
<td  ><p><strong>clearBigSQLResultSetMapMs</strong></p></td>
<td  ><p><strong>3.23.08</strong></p></td>
<td  ><p><strong>不兼容</strong></p></td>
<td  ><p><strong>原作为定时检测sql统计数据占用内存情况的周期（原配合useSqlStat=1、bufferUsagePercent使用）</strong></p></td>
</tr>
<tr>
<td  ><p><strong>移除</strong></p></td>
<td  ><p><strong>sqlRecordCount</strong></p></td>
<td  ><p><strong>3.23.08</strong></p></td>
<td  ><p><strong>不兼容</strong></p></td>
<td  ><p><strong>原作为慢查询show @@sql.slow记录阈值（原配合useSqlStat=1使用）</strong></p></td>
</tr>
<tr>
<td  ><p><strong>变更</strong></p></td>
<td  ><p><strong>samplingRate</strong></p></td>
<td  ><p><strong>3.23.08</strong></p></td>
<td  ><p><strong>兼容</strong></p></td>
<td  ><p><strong>修改默认值；原先为0，现在为100; 表示开启采样率为100%的sql统计，对应管理端sql_log表</strong></p></td>
</tr>
<tr>
<td  ><p><strong>新增</strong></p></td>
<td  ><p><strong>enableStatisticAnalysis</strong></p></td>
<td  ><p><strong>3.23.08</strong></p></td>
<td  ><p><strong>兼容</strong></p></td>
<td  ><p><strong>作为show @@sql.sum.user、show @@sql.sum.table、show @@sql.condition聚合统计的开关，默认不开启</strong></p></td>
</tr>
</tbody>
</table>

####  <span id="manager-cmd-3.23.08.0">2 管理端命令调整</span>
##### 2.1 sql统计相关show命令
...
+ 调整之前：
  1. 以上8个show命令均由`useSqlStat`开关参数控制。（与`samplingRate`参数、管理端sql_log表无关）
  2. show @@sql、show @@sql.high、show @@sql.slow、show @@sql.large、show @@sql.resultSet 区分用户存储，比如show @@sql，每个用户存储最新的1024条sql；show @@sql.high, 每个用户存储近期执行1024条高频的sql。（原保存5*N*1024条sql统计的数据量，其中N表示用户个数）
  3. show @@sql、show @@sql.high、show @@sql.slow、show @@sql.large、show @@sql.resultSet 通过在后面追加true的方式清理数据；比如: show @@sql true; 或者使用reload @@user_stat命令统一清理这5个命令的全部数据
  4. show @@sql.slow 由reload @@sqlslow=? 设置符合慢日志的阈值

+ 调整之后：
  + show @@sql、show @@sql.high、show @@sql.slow、show @@sql.large、show @@sql.resultSet;
    1. 这5个命令查询出来的数据来源于管理端sql_log表；sql_log表由`samplingRate`开关参数和`sqlLogTableSize`表大小限制参数控制，因此show @@sql.xx相关命令从此版本开始由这两个参数控制。
    2. `sqlLogTableSize`参数默认控制管理端sql_log表中只保存最新的1024个事务数据；由于show @@sql.xx相关命令的数据都基于sql_log，因此对比之前版本，show @@sql.xx相关命令的数据不再区分用户存储。（现在仅保存1024个事务数据量）
    3. 不支持show @@sql.xx true; 统一清理数据的方式为：`truncate sql_log`
    4. show @@sql.slow与[慢日志](../2.Function/2.1_manager_cmd/2.1.15_slow_query.md) 统一配置和开关；调整为由`sqlSlowTime`参数控制和reload @@slow_query.time=?;
  + show @@sql.sum.user、show @@sql.sum.table、show @@sql.condition；
    1. 由`enableStatisticAnalysis`开关参数控制，默认关闭, 此外可使用动态开关命令enable/disable @@statisticAnalysis； 这3个show命令维护各自数据（表示有3份数据，与sql_log无关）。

####  <span id="update-sql_log-3.23.08.0">3 sql_log表结构调整</span>
增加字段result_size：表示结果集的字节大小

###  <span id="config-title-3.23.04.0">3.23.04.0 变更内容</span>
####  <span id="manager-cmd-3.23.04.0">1 配置调整</span>
...
MySQL官方认为***5.7.20<=版本<8.0.0***和***版本>=8.0.3***是一致的，除此之外其它的版本则认为是同一种情况。例如以下的配置情况：  
