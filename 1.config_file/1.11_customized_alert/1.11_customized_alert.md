## 1.11 自定义告警模块
+ [告警工作原理](#11111-%E5%91%8A%E8%AD%A6%E6%A8%A1%E5%9D%97%E7%9A%84%E5%8A%A0%E8%BD%BD)
 - 告警模块的加载
 - 告警发送和解除
 - 告警Interface Alert详解
 - 告警发送对象详解
...
并在此解释下alert和alertSelf方法的区别：alertSelf方法的调用输入中为ClusterAlertBean设定了alertComponentType和alertComponentId两个字段，而alertSelf方法则没有，设定适当的DBLEserver标识的动作交给Alert对象来完成
...
| 告警代码                               | 解释                                   |
| -------------------------------------- | -------------------------------------- |
| DBLE_WRITE_TEMP_RESULT_FAIL            | 写出中间结果集到文件失败               |
| DBLE_XA_RECOVER_FAIL                   | XA事务恢复失败                         |
| XA_READ_XA_STREAM_FAIL                 | 读取XA事务记录文件失败                 |
| DBLE_XA_READ_DECODE_FAIL               | 解析XA事务记录文件失败                 |
| DBLE_XA_READ_IO_FAIL                   | 读取XA事务记录文件失败                 |
| DBLE_XA_WRITE_IO_FAIL                  | 写XA事务记录文件失败                   |
| DBLE_XA_WRITE_CHECK_POINT_FAIL         | XA写出检查点信息失败                   |
| DBLE_XA_BACKGROUND_RETRY_FAIL          | XA后台重试提交失败                     |
| DBLE_REACH_MAX_CON                     | 节点连接数到达配置最大值，获取连接失败 |
| DBLE_TABLE_NOT_CONSISTENT_IN_DATAHOSTS | 表结构在不同数据节点中不一致           |
| DBLE_TABLE_NOT_CONSISTENT_IN_MEMORY    | 表结构在数据节点中和dble内存中不一致   |
| DBLE_GLOBAL_TABLE_COLUMN_LOST          | 全局表检查列不存在                     |
| DBLE_CREATE_CONN_FAIL                  | 创建连接到后端mysql的连接失败          |
| DBLE_DATA_HOST_CAN_NOT_REACH           | 后端连接创建不可达                     |
| DBLE_KILL_BACKEND_CONN_FAIL            | Kill后端连接执行失败                   |
| DBLE_NIOREACTOR_UNKNOWN_EXCEPTION      | NIO意外报错                            |
| DBLE_NIOREACTOR_UNKNOWN_THROWABLE      | NIO意外严重错误                        |
| DBLE_NIOCONNECTOR_UNKNOWN_EXCEPTION    | NIO后端连接创建器意外错误              |
| DBLE_TABLE_LACK                        | 配置中的表格未被创建                   |
| DBLE_GET_TABLE_META_FAIL               | 获取表格的创建语句失败                 |
| DBLE_TEST_CONN_FAIL                    | 测试后端连接可达性失败                 |
| DBLE_HEARTBEAT_FAIL                    | 心跳后端节点失败                       |
| DBLE_DATA_NODE_LACK                    | 缺少可用的dataNode节点                 |
