...
1. 通过此链接( https://github.com/actiontech/dble/releases )下载 3.20.02.0 版本的安装包
2. 解压 dble 的安装包并进入 dble 镜像的 Dockerfile 所在的目录：
```
mkdir -p $working_dir  
cd $working_dir  
tar -xvf actiontech-dble-$version.tar.gz  
cd $working_dir/docker-images/dble-image
``` 
